...
<div id="wrap" align="center">
	...
	<button type="button" id="listBtn" data-oper='list'>도서 목록</button>
	<c:if test="${member.lev == 'A'}">
		<button type="button" id="regBtn" onclick="location.href='/book/register'">도서 등록</button>
		<button type="button" id="modBtn" data-oper='modify'>도서 수정</button>
		<!-- <button type="button" id="button" data-oper='remove'>도서 삭제</button> -->
	</c:if>
</div>
	...
	function makeBookReviewContent(obj){
		...
		if(obj.list.length === 0){
			...
			$(list).each(function(i,obj){
				...
				/* 아이디 */
				/* bookReview_list += '<span class="id_span">'+ obj.id+'</span>'; */
				/* 닉네임 */
...
$.getJSON("/book/getImageList", {isbn : isbn}, function(arr) {
	...
	str += "<img src='/book/display?fileName=" + fileCallPath + "'></div>";
...
$(document).ready(function() {
	...
	/* $("button[data-oper='remove']").on("click", function() {
		deleteForm.attr("action", "/book/remove").submit();
	}); */
