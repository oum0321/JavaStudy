		...
		<tr>
			<c:if test="${member.lev == 'A'}">
				<td colspan="8" style="border: white; text-align: right">
					<input type="button" value="앨범 등록" id="regibtn" onclick="location.href='/album/register'">
				</td>
			</c:if>
		</tr>
	...
	<div class="col-lg-12">
		<form id = "searchForm" action="/album/list" method="get">
			<select name="type"  id="hoption">
	     		...
	     		<option value="CT" <c:out value="${pageMaker.cri.type eq 'CT'? 'selected': ''}"/>>카테고리/제목</option>
	     		<option value="TS" <c:out value="${pageMaker.cri.type eq 'TS'? 'selected': ''}"/>>제목/아티스트</option>
	     		<option value="CTS" <c:out value="${pageMaker.cri.type eq 'CTS'? 'selected': ''}"/>>카테고리/제목/아티스트</option>     		
     		</select> 
...
$(document).ready(function() {
	...
	$(".move").on("click",function(e){
		e.preventDefault();
		//추가
		actionForm.find("input[name='productNum']").remove();  //뒤로 간 후 리스트 클릭하면 bno가 계속 쌓이는 문제
