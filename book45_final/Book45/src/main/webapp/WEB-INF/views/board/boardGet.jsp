...
<title>BOOK45 자유게시판 상세페이지</title>
...
$(document).on('click', '.update_reply_btn', function(e){
	...
	let popUrl = "/board/boardReplyUpdate?replyNum=" + replyNum + "&boardNum=" + '${board.num}' + "&memberId=" + '${member.id}';	
	let popOption = "width = 490px, height=490px, top=300px, left=300px, scrollbars=yes"	
...
function makeReplyContent(obj) {
	...
	if(obj.list.length === 0){
		...
		const boardWirter = '${board.memberId}';
		console.log('boardWirter: '+boardWirter);
		...
		$(list).each(function(i,obj){
			if(obj.secret=== false){
				...
				/* 아이디 */
				reply_list += '<div id="textspan"><span class="id_span">'+ obj.memberId+'</span>';
				...
				reply_list += '<div class="reply_top">';
				/* 아이디 */
				if(obj.memberId === userId || userId === boardWirter || memberLev === 'A'){
					reply_list += '<div id="textspan"><span class="id_span">'+ obj.memberId+'</span>';
				}else{
					reply_list += '<div id="textspan"><span class="id_span"></span>';
				}
				/* 날짜 */
				reply_list += '&#124;<span class="date_span">'+ obj.replyDate +'</span></div>';
...
$(document).ready(function() {
    ...
    $("button[data-oper='modify']").on("click", function(e) {
	   password = prompt("게시글 암호를 입력해주세요.(최대 4글자)", "암호입력");
	   
	   if (password != ${board.pass}) {
		   alert("암호가 틀렸습니다.");
		   return false;
	   } else {
		   alert("암호가 일치합니다 페이지를 이동 합니다.");
