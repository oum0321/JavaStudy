...
<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.6.3/jquery.min.js"></script>
   ...
   <div id="wrap" align="center">
      ...
      <table class="list" align="center">
         ...
         <tr>
            <th><input type="checkbox" class="allCheck" checked="checked"></th> <th>이미지</th> <th>제목</th> <th>수량</th> <th>가격</th> <th>삭제</th><!-- <th>이메일</th> <th>등급</th> <th>수정</th> <th>삭제</th> -->
         </tr>
         <c:forEach var="cart" items="${cartList}">
            <tr class="record">
            	<td class="cartInfoId">
            		...
            		<input type="hidden" name="id" class="id" value="${member.id}">
				    <input type="hidden" name="stock" id="stock" class="stock" value="<%= session.getAttribute("bookStock") %>">
				    <input type="hidden" name="albumStock" id="stock" class="stock" value="<%= session.getAttribute("albumStock") %>">
            	</td>
            <c:choose>
      ...
      <c:if test="${listCheck == 'empty'}">
       	<table class="list">
       	<tr>
       		<th><input type="checkbox" class="allCheck" checked="checked"></th> <th>이미지</th> <th>제목</th> <th>수량</th> <th>가격</th> <th>삭제</th>
       	</tr>
       	<tr>
       	<td id="line" colspan="6">장바구니가 비어 있습니다.</td>
       	</tr>
       	</table>
     </c:if> 	
...
$(".plusBtn").on("click", function() {
	...
	if (amount <= stock) {
		...
		$(this).parent("div").find("input").val(stock); 
	}
});
